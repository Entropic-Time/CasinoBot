...
import { Command, CommandDeferType } from '../interactions/index.js';
...
export class CommandHandler implements EventHandler {
    ...
    public async process(intr: CommandInteraction | AutocompleteInteraction): Promise<void> {
        ...
        try {
            ...
            let userData = await userSchema.getOrCreateUserData(intr.user.id, intr.user.username);
            userData.level = Number((userData.level + (1 / (userData.level * 10))).toFixed(3));
            userSchema.setUserData(userData);
        } catch (error) {
            console.log(error)
            Logger.error(
                intr.channel instanceof TextChannel ||
