...
export class Lang {
    ...
    public static getRawData(
        ...
        return (
            this.linguini.getRaw(location, langCode, variables) ??
            this.linguini.getRaw(location, Language.Default, variables)
        );
    }
    ...
    public static getRawLocalizationMap(
        location: string,
        variables?: { [name: string]: string }
    ): { [key: string]: any } {
        let obj = {};
        for (let langCode of Language.Enabled) {
            obj[langCode] = this.getRawData(location, langCode, variables);
        }
        return obj;
    }
